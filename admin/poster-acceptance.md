# Приёмочные тесты динамических плакатов

Дата: 2026-08-29. Автоматические тесты: `spring-posters` JUnit (HMAC, wrap, hash, PNG, map colors) и `auth-bot/posters-api/tests/run.php`.

Ручной прогон на live через публичный Velocity (`185.9.145.210:32073`) после
рестарта. Не подключайтесь напрямую к backend Paper:

| # | Сценарий | Ожидание | Статус |
|---|---|---|---|
| 1 | Игрок `/poster create`, текст в диалоге, Сохранить | На стене чистая бумага; на вылете бумага+текст+головы как один лист | pending live |
| 2 | Два других игрока смотрят без релога и без нового ресурспака | Каждый со сборкой лаунчера видит тот же текст на своём вылете | pending live |
| 3 | Рестарт сервера | Изображение на месте | pending live |
| 4 | Unload/load чанка | Чистая модель и сохранённый контент восстанавливаются | pending live |
| 5 | Три разных головы, шаблон `faces` | Все три нарисованы на вылетающем листе и качаются вместе с ним | pending live |
| 6 | Несуществующий ник | Ошибка в GUI, редактор жив | pending live |
| 7 | Длинный текст | Перенос, не вылезает за 85px | covered by wrap tests |
| 8 | Два редактора сразу | Второй видит «редактирует &lt;ник&gt;»; revision check | pending live |
| 9 | Чужой свой плакат обычным игроком | Отказ | pending live |
| 10 | Admin `poster.admin` | Открывает административный и чужой | pending live |
| 10a | `edit.any` без `poster.admin` | Свой/чужой игрока — да; административный — отказ | pending live |
| 10b | Shift+ПКМ по poster1 / oldposter / paperposter | Тот же редактор; чернила в поле своей текстуры | pending live |
| 10c | Игрок ломает опору админского плаката, TNT, поршень | Блок целый, плакат на месте | pending live |
| 11 | Renderer выключен | Старая картинка, ошибка, повтор | pending live |
| 12 | `/poster force` | Клиентские сессии сняты, стены чистые, без дублей | pending live |
| 13 | `/poster remove all` | Нет сущностей, нет строк БД | pending live |
| 14 | Повторное сохранение | Тот же `map_id`, revision++ | unit: hash reuse |
| 15 | Предметы GUI | Курсор не теряет стек | pending live |
| 16 | Старые настенные виды | Редактор открывается; сетка как у своей full-текстуры | pending live |
| 17 | Ванильный клиент без мода лаунчера | На стене и на вылете только чистая бумага, без текста | pending live |
| 18 | Git | Нет паролей, `.env`, db, generated PNG | checked before push |

Секреты в diff проверяются поиском паролей, `.env`, API secret values и служебных URI — без коммита `.cursor/rules`, `.env` и `posters.db`.
