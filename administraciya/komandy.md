...
`/decoratedpot spawn` ставит полноразмерную интерактивную вазу на блок под прицелом. Игроки поднимают её через Shift+ПКМ, бросают ЛКМ по физической дуге (не дальше 4 блоков) и отпускают ПКМ — ваза падает строго вниз без инерции и надёжно встаёт поверх блока.
